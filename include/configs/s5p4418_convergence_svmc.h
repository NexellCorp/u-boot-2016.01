...
#define CONFIG_SYS_PROMPT			"s5p4418_convergence_svmc# "
#endif
...
#define CONFIG_EXTRA_ENV_SETTINGS	\
	...
	"load_fdt="			\
		...
		"until test $loop -eq 0 || test $success -ne 0; do "	\
			...
			"else number=$loop; "				\
			"fi; "						\
			"ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-convergence_svmc-rev${number}.dtb && setexpr success 1; " \
			"setexpr loop $loop - 1; "			\
			"done; "					\
		"if test $success -eq 0; then "				\
			"ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-convergence_svmc-rev00.dtb;"	\
		"fi; "							\
	...
	"boot_cmd_mmcboot="   \
		"check_hw;ext4load mmc ${rootdev}:${bootpart} $kerneladdr $kernel_file;run load_fdt;run set_camera_input;" \

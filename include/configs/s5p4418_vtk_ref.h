 ...
 * Youngdo, Lee <ydlee@nexell.co.kr>
 *
...
#define CONFIG_SYS_PROMPT			"s5p4418_vtk_ref# "
#endif
	...
	"load_fdt=" \
		"if test -z \"$fdtfile\"; then " \
			...
			"until test $loop -eq 0 || test $success -ne 0; do " \
				...
				"else number=$loop; " \
				"fi; " \
				"ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-vtk-rev${number}.dtb && setexpr success 1; " \
				"setexpr loop $loop - 1; " \
				"done; " \
			"if test $success -eq 0; then " \
				"ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-vtk-rev00.dtb;" \
			"fi; "							\

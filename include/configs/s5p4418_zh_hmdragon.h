...
#define CONFIG_SYS_PROMPT			"s5p4418_zh_hmdragon# "
#endif
 ...
 */
#define CONFIG_SERIAL_MCU           1 /* set serial_2 to communicate with mcu */
#define CONFIG_CONS_INDEX_MCU       2
#define CONFIG_BAUDRATE_MCU         38400
...
#define CONFIG_EXTRA_ENV_SETTINGS	\
	...
	"load_fdt="			\
		...
		"until test $loop -eq 0 || test $success -ne 0; do "	\
			...
			"else number=$loop; "				\
			"fi; "						\
			"ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-zh_hmdragon-rev${number}.dtb && setexpr success 1; " \
			"setexpr loop $loop - 1; "			\
			"done; "					\
		"if test $success -eq 0; then "				\
			"ext4load mmc $rootdev:$bootpart $fdtaddr s5p4418-zh_hmdragon-rev00.dtb;"	\
		"fi; "							\

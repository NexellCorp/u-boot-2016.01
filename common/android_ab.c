				       ...
				       struct andr_bl_control **abc)
{
	...
	*abc = malloc_cache_aligned(abc_blocks * part_info->blksz);
	if (!*abc)
        ...
        printf("[debug][suker][%s]7\n", __func__);
        printf("[debug][suker][%s] part_info->start = 0x%lx\n", __func__, part_info->start);
        printf("[debug][suker][%s] abc_offset = 0x%lx\n", __func__, abc_offset);
        printf("[debug][suker][%s] abc_blocks = 0x%lx\n", __func__, abc_blocks);
